...
export const useAuthStore = defineStore("auth", () => {
  ...
  // 带参数的登录方法
  const enterLoginWithData = async (username: string, password: string, role: string) => {
    try {
  ...
  // 带参数的注册方法
  const enterRegisterWithData = async (username: string, password: string, role: string) => {
    try {
      const data = await register(username, password, role);
      ElMessage.success("注册成功，请登录");
      // 注册成功后不自动登录，返回数据供组件处理
      return data;
    } catch (error: any) {
      console.error(error?.message);
      throw error;
