...
const handleLogin = async () => {
  ...
  try {
    ...
    // 直接使用组件内的表单数据
    const data = await authStore.enterLoginWithData(loginForm.username.trim(), loginForm.password.trim(), 'teacher')
    console.log('✅ 教师登录成功:', data)
  } catch (error: any) {
...
const handleRegister = async () => {
  ...
  try {
    authStore.setRole('teacher')
    // 直接使用组件内的表单数据
    const data = await authStore.enterRegisterWithData(registerForm.username, registerForm.password, 'teacher')
